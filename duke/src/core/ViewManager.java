...
public class ViewManager {
	...
	public void shiftScreenX(int delta) {
		if (screenShiftX + delta < 0
				|| (screenShiftX + delta) / Settings.TILE_SIZE + Settings
						.SCREEN_WIDTH_IN_TILES() > Settings.WORLD_WIDTH)
			return;
		screenShiftX += delta;
	}
	...
	public void shiftScreenY(int delta) {
		if (screenShiftY + delta < 0
				|| (screenShiftY + delta) / Settings.TILE_SIZE + Settings
						.SCREEN_HEIGHT_IN_TILES() > Settings.WORLD_HEIGHT)
			return;
		screenShiftY += delta;
	}
	...
	private void drawWorld(Graphics2D g) {
		...
		for (int x = xstart; x < xstart + screenWidth + 1; x++) {
			for (int y = ystart; y < ystart + screenHeight + 1; y++) {
